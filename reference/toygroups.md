...
This data set serves to teach the concept of modelling species that
gather in groups and where the grouping behaviour depends on space.
...
- `df.intensity`::
  ...
  A `data.frame` with Poisson process intensity `d.lambda` at locations
  `x`

- `df.rate`::

  A `data.frame` the locations `x` and associated `rate` which
  parameterized the exponential distribution from which the group sizes
  were drawn.
