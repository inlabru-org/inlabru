...
test_that("flat SpatialPolygons integration", {
  ...
  ips0 <- bru_int_polygon(mesh, samplers = poly, nsub = 0)
  ips9 <- bru_int_polygon(mesh, samplers = poly, nsub = 9)

  expect_equal(sum(ips0$weight), 4)
  expect_equal(sum(ips9$weight), 4)
})
...
test_that("globe polygon integration", {
  ...
  ips0 <- inlabru:::mesh_triangle_integration(mesh, nsub = 0)
  ips9 <- inlabru:::mesh_triangle_integration(mesh, nsub = 9)

  expect_equal(sum(ips0$weight), 4 * pi * 6370.997^2)
})
