...
#' @describeIn ibm_names Set mapper names.
#' @export
#' @family mapper methods
#' @inheritParams ibm_n
...
format.bm_multi <- function(x, ..., prefix = "", initial = prefix, depth = 1) {
  txt <- NextMethod()
  if (depth <= 0) {
    return(txt)
  }
...
#' @rdname bm_summary
format.bm_pipe <- function(x, ..., prefix = "", initial = prefix, depth = 1) {
  txt <- NextMethod()
  if (depth <= 0) {
    return(txt)
  }
...
#' @rdname bm_summary
format.bm_sum <- function(x, ..., prefix = "", initial = prefix, depth = 1) {
  txt <- NextMethod()
  if (depth <= 0) {
    return(txt)
  }
...
format.bm_repeat <- function(x, ..., prefix = "", initial = prefix, depth = 1) {
  txt <- NextMethod()
  sub_prefix <- paste0(prefix, "      ")
  txt <-
    paste0(
...
print.bru_mapper <- function(
  ...
  cat(
    format(x, prefix = "", initial = prefix, depth = depth),
    sep = sep
  )
  invisible(x)
}
...
#' @export
#' @describeIn ibm_n_output Returns `NROW(input)`
ibm_n_output.default <- function(
  mapper,
  input,
  state = NULL,
  inla_f = FALSE,
...
#' @describeIn ibm_inla_subset
#' Uses the [ibm_values`()] output to construct the inla subset indexing as the
#' difference between `inla_f=FALSE` and `inla_f=TRUE`. Extra arguments such as
#' `multi` are passed on to [ibm_values()]. This means it supports both regular
...
#' @export
#' @rdname bm_fmesher
#' @param \dots Arguments passed on to [bm_fmesher()]
...
#' @describeIn bm_fmesher
#' Equivalent to calling [bm_fmesher()].
...
#' @export
#' @rdname bm_index
#' @param \dots Arguments passed on to [bm_index()]
...
#' @export
#' @rdname bm_taylor
#' @param \dots Arguments passed on to [bm_taylor()]
...
#'
ibm_n.bm_taylor <- function(mapper, inla_f = FALSE, multi = FALSE, ...) {
...
#' @rdname ibm_values
#'
ibm_values.bm_taylor <- function(mapper, inla_f = FALSE, multi = FALSE, ...) {
  if (inla_f && !is.null(mapper[["values_mapper"]])) {
...
#' @export
#'
ibm_eval.bm_taylor <- function(mapper, input = NULL, state = NULL, ...) {
  if (
    is.null(mapper[["jacobian"]]) ||
      (mapper[["n"]] == 0) ||
...
#' @export
#' @rdname bm_matrix
#' @param \dots Arguments passed on to [bm_matrix()]
...
#' or `sfc_POINT` object.
#'
ibm_jacobian.bm_matrix <- function(
  mapper,
  input,
  state = NULL,
  inla_f = FALSE,
...
#' @export
#' @rdname bm_factor
#' @param \dots Arguments passed on to [bm_factor()]
...
#' @export
#' @rdname bm_shift
#' @param \dots Arguments passed on to [bm_shift()]
...
#' @rdname ibm_n_output
#'
ibm_n_output.bm_shift <- function(
  mapper,
  input,
  state = NULL,
  ...,
...
#' @rdname ibm_values
#'
ibm_values.bm_shift <- function(mapper, ..., state = NULL, n_state = NULL) {
...
#' @export
#' @rdname bm_scale
#' @param \dots Arguments passed on to [bm_scale()]
...
#' @rdname ibm_n_output
#'
ibm_n_output.bm_scale <- function(
  mapper,
  input,
  state = NULL,
  ...,
...
#' @rdname ibm_values
#'
ibm_values.bm_scale <- function(mapper, ..., state = NULL, n_state = NULL) {
  n_state <- ibm_n(mapper, state = state, n_state = n_state)
  if (is.na(n_state)) {
    # Don't know how big the mapper will be
    NULL
...
#'
bm_aggregate <- function(rescale = FALSE, n_block = NULL, type = NULL) {
  if (!is.null(type)) {
    type <- match.arg(
      type,
...
#' @export
#' @param \dots Arguments passed on to [bm_aggregate()]
#' @rdname bm_aggregate
...
#' @rdname ibm_n_output
#'
ibm_n_output.bm_aggregate <- function(mapper, input = NULL, ...) {
...
#' @rdname ibm_values
#'
ibm_values.bm_aggregate <- function(mapper, ..., state = NULL, n_state = NULL) {
  n_state <- ibm_n(mapper, state = state, n_state = n_state)
  if (is.na(n_state)) {
    # Don't know how big the mapper will be
    NULL
...
#' ibm_eval2(m, list(block = c(1, 2, 1, 2), weights = 1:4, n_block = 3), 11:14)
#'
bm_logsumexp <- function(rescale = FALSE, n_block = NULL) {
...
#' @export
#' @rdname bm_logsumexp
#' @param \dots Arguments passed on to [bm_logsumexp()]
...
#'
ibm_eval.bm_logsumexp <- function(
  mapper,
  input,
  state = NULL,
  log = TRUE,
...
#'
ibm_eval.bm_logitaverage <- function(
  mapper,
  input,
  state = NULL,
  logit = TRUE,
...
require_args <- function(fun, req) {
  ...
  if (!all(subset)) {
    ...
    stop(paste0(
      "The '",
      label,
      "' function is missing the argument",
...
#' @export
#' @rdname bm_marginal
#' @param \dots Arguments passed on to [bm_marginal()]
...
#' @rdname ibm_n
#'
ibm_n.bm_marginal <- function(mapper, ..., state = NULL, n_state = NULL) {
  # Output size depends on the state size
  if (!is.null(state)) {
    length(state)
  } else if (is.null(n_state)) {
...
#' @rdname ibm_n_output
#'
ibm_n_output.bm_marginal <- function(
  mapper,
  input,
  state = NULL,
  ...,
...
#' @rdname ibm_values
#'
ibm_values.bm_marginal <- function(mapper, ..., state = NULL, n_state = NULL) {
  n_state <- ibm_n(mapper, state = state, n_state = n_state)
  if (is.na(n_state)) {
    # Don't know how big the mapper will be
    NULL
...
#'
ibm_jacobian.bm_marginal <- function(
  mapper,
  input,
  state = NULL,
  ...,
...
#' @export
#' @rdname bm_pipe
#' @param \dots Arguments passed on to [bm_pipe()]
...
#' @export
#' @rdname bm_multi
#' @param \dots Arguments passed on to [bm_multi()]
bru_mapper_multi <- function(...) {
  bm_multi(...)
}
...
#' @rdname ibm_values
#'
ibm_values.bm_multi <- function(mapper, inla_f = FALSE, multi = FALSE, ...) {
  if (multi) {
    vals <- if (inla_f) {
...
#' @export
#'
ibm_jacobian.bm_multi <- function(
  mapper,
  input,
  state = NULL,
  inla_f = FALSE,
...
#' @export
#'
ibm_invalid_output.bm_multi <- function(
  mapper,
  input,
  state,
  inla_f = FALSE,
...
#' @export
#' @rdname bm_harmonics
#' @param \dots Arguments passed on to [bm_harmonics()]
...
#' @rdname ibm_jacobian
#'
ibm_jacobian.bm_harmonics <- function(
  mapper,
  input,
  state = NULL,
  inla_f = FALSE,
