...
#' isn't supported.
#'
#' Used `ggmap::get_map()` to query map services like Google Maps for a region centered around
...
#' @describeIn inlabru-deprecated
#' `r lifecycle::badge("deprecated")` This function is deprecated as `ggmap` isn't supported.
#'
...
#'                `ggplot2::aes(x = .data[[sp::coordnames(data)[1]]],
...
#' @param mapping Aesthetic mappings created by `aes` used to update the default
#'                mapping. The default mapping is `ggplot2::aes(x = .data[[sp::coordnames(data)[1]]],
#'                y = .data[[sp::coordnames(data)[2]]], fill = .data[[names(data)[[1]]]])`.
#' @param crs A [sp::CRS] object defining the coordinate system to project the data to before plotting.
#' @param mask A [sp::SpatialPolygons] object defining the region that is plotted.
#' @param ... Arguments passed on to `geom_tile`.
