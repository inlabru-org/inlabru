...
#' data and the arguments passed on via \code{...}. gm is intended to replace gg whenever the
...
#' @export
#' @import ggplot2
#' @param data A SpatialGrid object
...
gg.SpatialGrid = function(data, fill = names(data)[[1]], ...) {
  df <- as.data.frame(data)
  gm = geom_tile(data = df, mapping = aes_string(x = coordnames(data)[1], y = coordnames(data)[2], fill = fill), ...)
...
gg.SpatialPixelsDataFrame = function(data, fill = names(data)[[1]], alpha = NULL, mask = NULL, ...) {
  ...
  df <- as.data.frame(data)
  dmap = aes_string(x = coordnames(data)[1], y = coordnames(data)[2], fill = fill)
...
gg.inla.mesh.1d = function(data, y = 0, shape = 4, ...) {
