...
#'   geom_point(data = countdata3a, aes(x = x), y = 0, shape = "+",
#'              col = "blue", cex = 4) +
#'   geom_point(data = data.frame(x = countdata3a$x, y = E_nc3a),
#'              aes(x = x), y = E_nc3a, shape = "_", cex = 5) +
#'   xlab(expression(bold(s))) + ylab("count")
#' ss <- seq(0, 55, length = 200)
#' lambda <- lambda3_1D(ss)
#' p2a <- ggplot() +
#'   geom_line(data = data.frame(x = ss, y = lambda), aes(x = x, y = y),
#'             col = "blue") +
#'   ylim(0, max(lambda)) +
...
#'   geom_point(data = countdata3b, aes(x = x), y = 0, shape = "+",
#'              col = "blue", cex = 4) +
#'   geom_point(data = data.frame(x = countdata3b$x, y = E_nc3b),
