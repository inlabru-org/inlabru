...
Repeat mappers take a single mapper and repeats it, by defining the Jacobian
as a Kronecker product of a row vector of ones, corresponding to a state vector
of length `nrep * ibm_n(mapper, ...)`. The `ibm_` method inputs are
...
* `fm_mesh_2d` and `inla.mesh`; Use `bru_mapper(object)` or `bru_mapper_fmesher(object)`
* `fm_mesh_1d` and `inla.mesh.1d`; Use `bru_mapper(object, indexed)` or,
  if `indexed=TRUE`, `bru_mapper_fmesher(object)`.
* Any object supporting the `fmesher::fm_dof()` and `fmesher::fm_basis()` methods;
  Use `bru_mapper_fmesher(object)`. If `fmesher` in the future gains a common
