...
* `bru_mapper_scale()`
* `bru_mapper_marginal(qfun, pfun, param)`
...
```{r, eval=FALSE}
mapper <- bru_mapper_mapper(qfun = ..., pfun = ..., param = ...)
ibm_eval(mapper,
  input = NULL, # If a list is given, it overrides the 'param' specification
  state = ...
)

# Example:
mapper <- bru_mapper_mapper(qfun = qexp, pfun = pexp, param = list(rate = 1 / 8))
```
