...
The wider scope of component definitions is discussed in the [component vignette](http://inlabru-org.github.io/inlabru/articles/component.html). Here, we
...
The in-package documentation for the mapper methods is contained in three parts:
```{r, eval=FALSE}
...
The `bru_mapper_define()` and `bru_get_mapper()` method is needed for those
implementing their own mapper
class.
...
The `index` mapper defines a direct mapping between a length-`n` state
...
```{r, eval=FALSE}
mapper <- bru_mapper_scale(inner_mapper)
ibm_eval(mapper,
         input = list(mapper = inner_mapper_input,
                      scale = ...),
         state)
```

All component mappers are currently defined as a `multi` mapper within a `scale`
mapper:
```{r, eval=FALSE}
inner_mapper <- bru_mapper_multi(list(main = ..., group = ..., replicate = ...))
mapper <- bru_mapper_scale(inner_mapper)
ibm_eval(mapper,
         input = list(mapper = list(main = ..., group = ..., replicate = ...),
                      scale = ...),
         state = list(main = ..., group = ..., replicate = ...)
```
...
* `inla.mesh`; Use `bru_mapper(object)`
* `inla.mesh.1d`; Use `bru_mapper(object, indexed)`
