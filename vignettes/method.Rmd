...
Let $\wt{\bm{\eta}}(\bm{u})$ be a non-linear predictor, i.e. a deterministic function of $\bm{u}$, $$ \wt{\bm{\eta}} (\bm{u}) = \mathsf{fcn} (\bm{u}), $$
and let
...
point $\bm{u}_*$. We start by
introducing a functional $f(\ol{p}_{\bm{v}})$ of the posterior
distribution linearised at $\bm{v}$,
that generates a latent field configuration. We then seek an optimal fixed point of the
functional, so that $\bm{u}_*=f(\ol{p}_{\bm{u}_*})$ such that 

$$
\begin{align}
    \wh{{\bm{\theta}}} &= \argmax_{\bm{\theta}} p_\bm{v} ( {\bm{\theta}} | \bm{y} ) \approx \argmax_{\bm{\theta}} \ol{p}_\bm{v} ( {\bm{\theta}} | \bm{y} ),  \label{eq:theta}\\
    \bm{u}_* &= \argmax_{\bm{u}} p_\bm{v} (\bm{u} | \bm{y}, \wh{{\bm{\theta}}}) \approx \argmax_{\bm{u}} \ol{p}_\bm{v} (\bm{u} | \bm{y}, \wh{{\bm{\theta}}}),  \label{eq:u_*}
\end{align}
$$
Combining both, it is equivalent to optimise the posterior modes $\wh{\bm{\theta}}$ and $\bm{u}_*$ via 
$$ \argmax_{{\bm{\theta}}, \bm{u}} p_\bm{v} ({\bm{\theta}}, \bm{u} | \bm{y}) \approx \argmax_{{\bm{\theta}}, \bm{u}} \ol{p}_\bm{v} ({\bm{\theta}}, \bm{u}|\bm{y}).$$

Potential choices for $f(\cdot)$ include the posterior expectation
$\ol{E}(\bm{u}|\bm{y})$ and the joint conditional mode,
$$
...
0. Let $\bm{u}_0$ be an initial linearisation point for the latent variables 
   obtained from the initial INLA call.
...
4. Let $\bm{u}_\alpha=(1-\alpha)\bm{u}_0+\alpha\bm{u}_1$, and find the value
   $\alpha$ minimises $\|\wt{\eta}(\bm{u}_\alpha)-\ol{\eta}(\bm{u}_1)\|$.
...
$$ \argmax_{\alpha} [\ln p_{\bm{u}_\alpha}(\bm{u}|\bm{y},\wh{{\bm{\theta}}})].$$
$\|\wt{\eta}(\bm{u}_\alpha)-\ol{\eta}(\bm{u}_1)\|$ is a loss function that an approximate line search can be used. It avoids many potentially
...
this is basically the approximate version of Kullback–Leibler divergence term. 
...
#### Initialisation
On a sidetrack, some may concern with an initalisation from a saddle point. although it is not implemented in inlabru, we want to talk about the technicality how we define the initial linearisation point $u_0$.
