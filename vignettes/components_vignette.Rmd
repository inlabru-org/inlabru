...
$$
\begin{equation}
\eta(u)_i = u_0 + \sum_{k=1}^K v_{ik} u_{ik} ,
\end{equation}
$$
where $u$ is a multivariate Gaussian vector, $v_k$ are input information such as covariates or weights for random effects and $i = 1, \ldots, n$.  This can also be written as $\eta(u) = Au$, where $A$ is the model design matrix with $i$-th row $\left[1, v_{i1}, \ldots, v_{iK}\right]$.

We can also conceputally think of the predictor as the sum of $D$ model components, so that we partition $A = \left[A^{(1)} \cdots A^{(D)}\right]$, and each component has an associated **component design matrix** $A^{(d)}$. 

For example, if the component is an intercept parameter, then $A^{(d)} = \left[1, \ldots, 1\right]^\intercal$. If the component is the linear effect of a covariate $z$ then $A^{(d)} = \left[z_1, \ldots, z_n\right]^\intercal$. For more complicated effects, such as SPDE models, the component design matrix maps latent Gaussian parameters to the predictor (also known as the "projector" matrix in this context).  The the construction of each $A^{(d)}$ is handled automatically by `bru_mapper` methods.
...
The appropriate weights required to evaluate the effect at the observation locations can be constructed using `INLA::inla.spde.make.A()`.  The mapper for this model component takes the information in the component definition, in this case the minimum information required is an SPDE model object, and the 2-column matrix that `main` is evaluated to. The mapper then calls `INLA::inla.spde.make.A()` with appropriate arguments extracted from this information.

(NOTE: Deliberately not going into huge detail here; the [bru_mapper](devel_bru_mapper.html) vignette will have more details.)

### Defining `main`, `group`, and `replicate`.

The arguments `main`, `group`, and `replicate` can all take a general `R` expression as an input.  This expression is then evaluated in an environment that consists of the named variables in the data (note: for `sp` objects this includes the column names from the `@coords` slot as well as the columns in `@data`).  
...
~ my_spde_effect(main = get_xy(.data.),
                 model = spde_model)
```
...
2. Using a covariate stored in an `sp` `Spatial*` object.
...
If `main`, `group`, or `replicate`, is the name of an `sp` object stored in the global `R` environment, then `inlabru` attempts to do something intelligent by extracting the covariate information at the locations of the data passed to `bru()` or `like()`.  In this case **it is assumed that this data is also an `sp` object**.  `inlabru` does this by applying `sp::coordinates()` to the data and then extracting the covariate information using `sp::over()`.

The shorcut
```{r eval=FALSE}
~ my_sp_effect(main = an_sp_object,
               model = "linear")
```
...
~ my_sp_effect(main = get_sp_covariate(.data.),
               model = "linear")
```
Note that this requires `an_sp_object` to be stored in the global `R` environment
(or in the environment associated with the model definition code) so it is findable when the expression is internally evaluated by `inlabru`.  Also note that the `get_sp_covariate` function above extract the first column of the `sp` object evaluation.  For more general situations, one can either specify the optional `main_layer` argument to extract another named or indexed column, or directly
use `main = eval_spatial(an_sp_object, .data., layer = some_layer)`.

Note that this assumes that `coordinates()` applied to the input data is a sensible thing to do, this might not be the case for all models!  For example, if the input data is a `SpatialPolygonsDataFrame` then `coordinates()` returns the centroid of each polygon.  As more specific input type support is developed,
...
```{r eval=FALSE}
~ my_fixed_effects(main = ~ x1:x2 + x3*x4,
                   model = 'fixed')
```
...
```{r eval=FALSE}
~ a_component(main = a_function,
              model = ...)
```
is equivalent to
```{r eval=FALSE}
~ a_component(main = a_function(.data.),
              model = ...)
```

#### Non-linear predictors

`inlabru` supports non-linear predictors, where $\tilde{\eta}(u)$ is a non-linear function of $u$.  It is important to note that the mapping to a predictor vector $Au$ happens **before** the non-linear function is applied.  So, for example, if $\tilde{\eta}(u) = \exp(u)$ then this is evaluated as `exp(A %*% u)` rather than `A %*% exp(u)`.
