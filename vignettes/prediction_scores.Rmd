...
date: "Corrections and code updates 2025-03-31"
...
inla_available <- bru_safe_inla(multicore = interactive(), quietly = TRUE)
```
...
```{r, eval=inla_available}
pred <- predict(fit, newdata, formula = ~ exp(Intercept + x), n.samples = 2000)
post_E <- pred$mean
post_Var <- pred$mean + pred$sd^2
scores <- data.frame(
  SE = (newdata$y - post_E)^2,
  DS = (newdata$y - post_E)^2 / post_Var + log(post_Var)
)
summary(scores)
```
...
head(
  data.frame(
    log_S = log_score,
    lower = log_score - 2 * pred$mean.mc_std_err,
    upper = log_score + 2 * pred$mean.mc_std_err
  )
)
```
...
max_K <- ceiling(max(newdata$y) + 4 * sqrt(max(newdata$y)))
k <- seq(0, max_K)
kk <- rep(k, times = length(newdata$y))
i <- seq_along(newdata$y)
...
results <- data.frame(
  ...
  k = kk,
  Fpred = rowMeans(pred),
  residuals = rowMeans(pred) - (rep(newdata$y, each = length(k)) <= kk)
)
...
use
```{r, eval=inla_available}
pred <- predict(fit, newdata, formula = ~ exp(Intercept + x))
scores <- (newdata$y - pred$mean)^2
```
...
```{r, eval=inla_available}
pred <- predict(fit, newdata, formula = ~ {
    lambda = exp(Intercept + x)
    list(
...
crps_score <-
  pred$crps$mean -
  (pred$F %>%
    mutate(i = rep(seq_along(newdata$y), each = max_K + 1)) %>%
    group_by(i) %>%
