...
supplying data for the other components).  From version `2.8.0`, `inlabru`
attempts to automatically detect which model components are used in the
expression, and the `include` argument can be left out entirely.

Since the elevation effect in this model is linear, the resulting plot isn't
very interesting, but the same method can be applied to non-linear effects as
well, and combined into general R expressions.
