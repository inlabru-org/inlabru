...
author: "Niharika Reddy Peddinenikalva, with sf updates by Finn Lindgren"
date: "`r Sys.Date()`"
...
if (file.exists(code_file)) {
  ...
  code_location <- system.file("misc", code_file, package = "inlabru")
...
### Scaling Residuals
By taking $h(u) \equiv 1$, we get the raw residuals:
$$
\begin{aligned}
    R(B, \hat{h}, \hat{\theta}) & = I(B, \hat{h}, \hat{\lambda}) = 
    \sum_{x_i \in \vb{x} \cap B}h(x_i) - \int_B \hat{h}(u)\hat{\lambda}(u)\,du\\
...
$$
\begin{aligned}
\text{Scaled:} && R(B, 1, \hat{\theta}) &= n(\vb{x} \cap B) - 
\int_B \hat{\lambda}(u) \,du \\
\text{Inverse:} && R\left(B, \frac{1}{\hat{\lambda}}, \hat{\theta}\right) 
&= \sum_{x_i \in \vb{x} \cap B} \frac{1}{\hat{\lambda}(x_i)} - 
\int_B \vb{1} \{ \hat{\lambda}(u) > 0 \} \,du \\
\text{Pearson:} &&  R\left( B, \frac{1}{\sqrt{\hat{\lambda}}},\hat{\theta}
...
respectively. 
Then, $h_s(u) \equiv 1$, $h_i(u) = \frac{1}{\lambda(u)}$,
...
elev <- gcov$elevation
terra::values(elev) <- terra::values(elev) -
  mean(terra::values(elev), na.rm = TRUE)
...
The code chunk below shows how the model is defined by `bru_obs()` and also 
