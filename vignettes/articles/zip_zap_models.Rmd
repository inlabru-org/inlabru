...
We compute four prediction scores, Absolute Error (AE), Squared Error (SE),
Dawid-Sebastiani (DS), and (negated) Log score (LG):

$$
\begin{aligned}
\text{AE}_i&=|y_i-\text{median}_i|,\\
\text{SE}_i&=[y_i-E(X_i)]^2, \text{ and}\\
\text{DS}_i&=\frac{[y_i-E(X_i)]^2}{V(X_i)} + \log[V(X_i)], \\
\text{LG}_i&=-\log[P(X_i = y_i)].
\end{aligned}
$$
...
the median and expectation, respectively. The (negated) Log score is a strictly
proper score. (insert ref here to Gneiting and Raftery, 2005(?))
