...
lambda <- predict(
  fit_poi, fm_pixels(px_mesh, mask = boundary),
...
lambda <- predict(
  fit_zip, fm_pixels(px_mesh, mask = boundary),
  ~ exp(vegetation + elevation + field + Intercept)
)
...
lambda <- predict(
  fit_zap, fm_pixels(px_mesh, mask = boundary),
  ~ exp(vegetation + elevation + field + Intercept)
)
...
In order to make sure that Poisson distribution will be properly truncated, we include the `control.family` argument with the parameter `theta` (which gets translated into the probability $p$) is fixed to a large negative value, such that when transformed by $\text{logit}^{-1}(\theta)=p$ it would end up close to zero.
