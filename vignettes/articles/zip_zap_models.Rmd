...
pred_zip <- predict(
  ...
  ~ {
    ...
    expect2 <- expect * (1 + (1 - scaling_prob) * lambda * area)
    list(
      lambda = lambda,
      expect = expect,
      expect2 = expect2 - expect^2
    )
  }
)
expect_zip <- pred_zip$expect
expect_zip$pred_var <- pred_zip$expect2$mean + expect_zip$sd^2
...
pred_zap <- predict(
  ...
  ~ {
    ...
    expect2 <- expect * (1 + presence_prob * lambda * area)
    list(
...
expect_zap$pred_var <- pred_zap$expect2$mean + expect_zap$sd^2
...
conditional mean and variance for $\lambda_i$. Note: this partially ignores the special zero-treatment; just as for the ZAP expectation, need to derive the proper predictive variance expressions for ZIP and ZAP. Also, the equation doesn't quite match the needed notation.
...
We compute three prediction scores,

$$\text{MAE}=\frac{1}{n}\sum_i |y_i-\text{median}_i|,$$
$$\text{RMSE}=\sqrt{\frac{1}{n}\sum_i (y_i-m_i)^2}, \text{ and}$$
$$\text{DS}=\frac{1}{n}\sum_i \left[\frac{\sum_i (y_i-m_i)^2}{v_i^2} + \log{v_i^2}\right],$$
where DS is the Dawid-Sebastiani score that is a proper scoring rule for the
predictive mean $m_i$ and variance $v_i^2$.

```{r, out.width="100%", fig.width=15, fig.height=5, fig.align='center'}
expect_poi$DS <- with(expect_poi, (counts_df$count - mean)^2 / pred_var + log(pred_var))
expect_zip$DS <- with(expect_zip, (counts_df$count - mean)^2 / pred_var + log(pred_var))
expect_zap$DS <- with(expect_zap, (counts_df$count - mean)^2 / pred_var + log(pred_var))

scores <- df %>%
  group_by(Model) %>%
  summarise(
    MAE = mean(abs(count - pred_median)),
    RMSE = sqrt(mean((count - pred_mean)^2))
  )
# Add the average Dawid-Sebastiani scores
scores <- full_join(scores,
  data.frame(
...
knitr::kable(scores %>% arrange(Order) %>% select(-Order))
```
We see that the MAE and RMS are comparable for the ZAP model and the other two models, but the DS score is clearly lower. That means that gives a better estimate of the prediction uncertainty.


```{r, out.width="100%", fig.width=15, fig.height=5, fig.align='center'}
p1 <- ggplot() +
  geom_fm(data = px_mesh) +
  gg(expect_poi, aes(fill = DS), geom = "tile") +
  geom_sf(data = nests, color = "firebrick", size = 1, pch = 4, alpha = 0.2) +
  ggtitle("Poisson Dawid-Sebastiani scores") +
  guides(fill = guide_legend("DS"))
p2 <- ggplot() +
  geom_fm(data = px_mesh) +
  gg(expect_zip, aes(fill = DS - expect_poi$DS), geom = "tile") +
  geom_sf(data = nests, color = "firebrick", size = 1, pch = 4, alpha = 0.2) +
  ggtitle("ZIP Dawid-Sebastiani score difference") +
  guides(fill = guide_legend("DS-DS_poi"))
p3 <- ggplot() +
  geom_fm(data = px_mesh) +
  gg(expect_zap, aes(fill = DS - expect_poi$DS), geom = "tile") +
  geom_sf(data = nests, color = "firebrick", size = 1, pch = 4, alpha = 0.2) +
