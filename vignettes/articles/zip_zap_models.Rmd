...
Next, we can define the Poisson model that links the species count per observation plot (raster cell) to the spatial covariates, such as vegetation type and elevation. To ensure identifiability of the vegetation effect estimates relative to the intercept, we use `model = "factor_full"` with a sum-to-zero constraint.
...
pred_poi <- predict(
  fit_poi, counts_df,
  ~ {
    expect <- exp(vegetation + elevation + field + Intercept) * area
    list(
      expect = expect,
