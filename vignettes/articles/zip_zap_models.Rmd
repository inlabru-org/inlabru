...
```{r fig-vegetation-raster, fig.cap="Proportion of vegetation cover by class", fig.width=7, fig.height=5, out.width="80%", fig.align='center'}
gcov_lvls <- gcov$vegetation %>% levels()
gcov_vegetation <- gcov$vegetation %>% segregate() %>%
  terra::aggregate(fact = 4, fun = mean)
names(gcov_vegetation)<- gcov_lvls[[1]]$vegetation
gcov_vegetation %>% plot()
#focal(w=matrix(1/25,nrow=5,ncol=5)) # alternative - convolution
```
...
<!-- To ensure identifiability of the vegetation effect estimates relative to the intercept, we use `model = "factor_full"` with a sum-to-zero constraint. -->
<!-- An alternative approach would be to use `model = "factor_contrast"` without a constraint, but this would require reordering factor levels to prevent the least common vegetation type from becoming the reference level. -->
...
pred_poi <- predict(
  ...
  ~ {
    expect <- exp(#vegetation + 
      veg_disturbed + veg_grassland + 
        elevation + field + Intercept) * area
    list(
      expect = expect,
...
pred_zip <- predict(
  ...
  ~ {
    ...
    list(
      ...
      expect = expect,
      variance = variance,
      obs_prob = (1 - scaling_prob) * (count == 0) + scaling_prob * dpois(count, expect)
    )
  },
  n.samples = 2500
)
...
We transition to the `zeroinflatedpoisson0` model, which is defined by the [following observation probability model](https://inla.r-inla-download.org/r-inla.org/doc/likelihood/zeroinflated.pdf)
...
p1 <- ggplot() +
  geom_fm(data = px_mesh) +
  gg(df_, aes(fill = LG), geom = "tile") +
  geom_sf(data = nests, color = "firebrick", size = 1, pch = 4, alpha = 0.2) +
  ggtitle("LG score") +
  guides(fill = guide_legend("LG"))
p2 <- ggplot() +
  geom_fm(data = px_mesh) +
  gg(df_, aes(fill = LG - LG_Poisson), geom = "tile") +
  geom_sf(data = nests, color = "firebrick", size = 1, pch = 4, alpha = 0.2) +
