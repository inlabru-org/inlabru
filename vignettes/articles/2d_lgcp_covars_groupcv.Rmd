...
```{r warning=FALSE,message=FALSE}
cvpart <- cv_hex(boundary, cellsize = 0.5, n_group = 3)
...
pred <- predict(fit_veg,
  ...
  formula = ~ {
    ...
    lambda_block_int <- ibm_eval(agg_block,
      ...
      state = (1 - exp(eta[part == "block_int"]))
    )
    block_log_like <- lambda_block_int +
      ibm_eval(agg_block,
...
IC_results <-
  data.frame(
    type = c("DIC", "WAIC_limit", "WAIC_blockwise", "DIC_INLA", "WAIC_INLA"),
    lppd = c(
      lppd_limit,
      lppd_limit,
      lppd_blockwise,
      fit_veg$dic$dic / (-2) + fit_veg$dic$p.eff + sum(int_points$weight),
      NA
    ),
    p_eff = c(
      p_DIC,
...
fit_veg2_prep <- bru_set_missing(
  fit_veg,
  keep = list(-which(as_bru_obs_list(fit1)[[1]]$response_data$BRU_block == 8L))
)
