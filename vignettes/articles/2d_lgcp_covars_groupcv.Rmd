...
bru_options_set(control.compute = list(dic = TRUE, waic = TRUE)) # Activate DIC output
```
...
To enable group.cv calculations, we need to split the domain into spatial regions,
and compute the association of each observed point to these subregions.

```{r warning=FALSE,message=FALSE}
source("hexagon_tiling.R")
...
```{r}
int_points <- fm_int(domain=list(geometry = mesh), boundary)
block_int_points <- fm_int(domain=list(geometry = mesh), cvpart)
...
pred <- predict(fit1,
        ...
        formula = ~ {
          ...
          lambda_block_int <- ibm_eval(agg_block,
                                       input = list(block = .block[part == "block_int"],
          ...
          list(log_lambda_y = log_lambda_y,
               lambda_y = lambda_y,
...
IC_results <- 
  data.frame(
    ...
    lppd = c(lppd_limit,
             lppd_limit,
             lppd_blockwise,
             fit1$dic$dic/(-2) + fit1$dic$p.eff + sum(int_points$weight),
             NA),
    p_eff = c(p_DIC, p_WAIC_limit, p_WAIC_blockwise, fit1$dic$p.eff, fit1$waic$p.eff),
    IC = c(DIC,
           WAIC_limit,
           WAIC_blockwise,
           fit1$dic$dic,
...
```{r}
fit2prep <- bru_set_missing(fit1, keep = list(-which(fit1$bru_info$lhoods[[1]]$response_data$BRU_block == 8L)))
fit2 <- bru_rerun(fit2prep)
```
