...
ggplot() +
  gg(sf::st_as_sf(mexdolphin$ppoly)) +
...
fit <- lgcp(
  components = cmp,
  sf::st_as_sf(mexdolphin$points),
  samplers = sf::st_as_sf(mexdolphin$samplers),
  domain = list(
    geometry = mesh,
...
pxl <- fm_pixels(mesh, nx = 200, ny = 100, mask = mexdolphin$ppoly, format = "sf")
pr.int <- predict(fit, pxl, ~ exp(mySPDE + Intercept))

ggplot() +
  gg(pr.int, aes(fill = mean), geom = "tile") +
  gg(mexdolphin$ppoly) +
  gg(mexdolphin$samplers, color = "grey") +
  gg(mexdolphin$points, size = 0.2, alpha = 1) +
  theme(legend.key.width = unit(x = 0.2, "cm"), legend.key.height = unit(x = 0.3, "cm")) +
...
From version `2.8.0`, inlabru attmepts to automatically detect which components
are used by the predictor expression, and the `include` argument can usually be left out.
...
predpts <- fm_int(mexdolphin$mesh, mexdolphin$ppoly, format = "sf")
...
ggplot() +
  gg(pr.int1, aes(fill = mean), geom = "tile") +
  gg(mexdolphin$ppoly) +
  gg(mexdolphin$samplers, color = "grey") +
  gg(mexdolphin$points, size = 0.2, alpha = 1) +
  theme(legend.key.width = unit(x = 0.2, "cm"), legend.key.height = unit(x = 0.3, "cm")) +
