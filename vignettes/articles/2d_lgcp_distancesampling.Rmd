...
```
To control the prior distribution for the `sigma` parameter, we use a transformation mapper
...
```
Here, the range is probabilistically limited by $P(\text{range}\leq 50)=0.01$ and the standard deviation of the spatial field is limited by $P(\text{sd}\leq 2)=0.01$.
...
cmp <- ~ mySPDE(main = geometry, model = matern) +
  ...
  Intercept(1)
```
The `marginal` argument in the `sigma` component specifies the transformation function
taking N(0,1) to Exponential(1/8).

The formula, which describes how these components are combined to 
...
Before version `2.9.0.9004`, a less compact approach had to be used, by applying the transformation
between N(0,1) and Exponential(1/8) directly in the predictor expression, requiring
corresponding adjustments to the later `predict()` calls, etc:
