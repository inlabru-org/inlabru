...
```{r eval=TRUE}
mapper <- bru_mapper_collect(list(a = bru_mapper_index(3),
...
```{r eval=TRUE}
mapper <- bru_mapper_multi(list(a = bru_mapper_index(3),
                                b = bru_mapper_index(2)))
ibm_n(mapper)
...
$$
\eta_i = x_0 + \sum_{j=1}^p a_{ij} x_j +  \sum_{j=1}^p\sum_{k=1}^j a_{ij}a_{ik} x_{j,k},
$$
where the latent component vector is $[x_0,x_1,\dots,x_p,x_{1,1},\dots,x_{p,1},x_{2,2}\dots,x_{p,p}]$.
...
ibm_values.bru_mapper_p_quadratic <- function(mapper, ...) {
  ...
  c(if (mapper$min_degree <= 0) "Intercept" else NULL,
