...
ggplot() + gg(mesh) + geom_point(aes(loc[,1], loc[,2])) + coord_equal()
```
...
mapper <- bru_mapper_multi(list(main = bru_mapper(mesh),
...
mapper <- bru_mapper_pipe(
  list(main = bru_mapper_multi(list(main = bru_mapper(mesh), ...)),
       block = bru_mapper_aggregate(rescale = (block_rescale != "none"),
                                    n_block = n_block))
)
ibm_jacobian(mapper,
             input = list(
...
mapper <- bru_mapper_multi(list(field.main = bru_mapper(mesh),
