...
```{r load-functions, code=readLines("2d_lgcp_residuals_functions.R"), eval=TRUE, echo=FALSE}
...
As <- prepare_residual_calculations(samplers = B, domain = mesh,
                     observations = nests)
```
...
fit1 <- lgcp(comp1, nests, samplers = boundary, 
             domain = list(coordinates = mesh))

# Display the model
int1 <- predict(fit1, data = pixels(mesh, mask = boundary), 
...
veg_res <- residual_df(fit1, As$df, expression(exp(vegetation + Intercept)), 
...
fit2 <- lgcp(comp2, nests, samplers = boundary, 
             domain = list(coordinates = mesh))

# Display the model
int2 <- predict(fit2, pixels(mesh, mask = boundary), 
...
fit3 <- lgcp(comp3, nests, samplers = boundary, 
             domain = list(coordinates = mesh))

# Display the model
int3 <- predict(fit3, pixels(mesh, mask = boundary), ~ exp(Intercept))
ggplot() +
  gg(int3) +
  gg(boundary, alpha = 0) +
  gg(nests, shape = "+") +
...
# Calculate the residuals for the model
int_res <- residual_df(model = fit3, df = As$df, 
            expr = expression(exp(Intercept)), 
            A_sum = As$A_sum, A_integrate = As$A_integrate)
knitr::kable(edit_df(int_res, 
                     c("Type", "mean.mc_std_err", "sd.mc_std_err", "median")))
```
...
comp4 <- coordinates ~ mySmooth(coordinates, model = matern) + 
  Intercept(1)
fit4 <- lgcp(comp4, nests, samplers = boundary, 
             domain = list(coordinates = mesh))

# Display the model
int4 <- predict(fit4, pixels(mesh, mask = boundary), ~exp(mySmooth + Intercept))
ggplot() +
  gg(int4) +
  gg(boundary) +
  gg(nests, shape = "+") +
...
# Calculate the residuals for the model
smooth_res <- residual_df(model = fit4, df = As$df, 
            expr = expression(exp(mySmooth + Intercept)), 
            A_sum = As$A_sum, A_integrate = As$A_integrate)
knitr::kable(edit_df(smooth_res, 
                     c("Type", "mean.mc_std_err", "sd.mc_std_err", "median")))
```
### Comparing models
...
As1 <- prepare_residual_calculations(samplers = B1, domain = mesh, 
                                     observations = nests)
# Residuals for the vegetation model
veg_res2 <- residual_df(fit1, As1$df, expression(exp(vegetation + Intercept)),
                        As1$A_sum, As1$A_integrate)
knitr::kable(edit_df(veg_res2, c("Type", "mean.mc_std_err", 
...
As2 <- prepare_residual_calculations(samplers = B2, domain = mesh, 
                                     observations = nests)
# Residuals for the vegetation model
veg_res3 <- residual_df(fit1, As2$df, expression(exp(vegetation + Intercept)),
...
Consider two more resolutions of $B$, given in the plots below.
...
As3 <- prepare_residual_calculations(samplers = B3, domain = mesh, 
                                     observations = nests)
...
# Calculate preparatory data for residuals at this resolution
As4 <- prepare_residual_calculations(samplers = B4, domain = mesh, 
                                     observations = nests)
```
...
B3res <- rbind(Residual_fit1_B3, Residual_fit2_B3, 
               Residual_fit3_B3, Residual_fit4_B3)
B4res <- rbind(Residual_fit1_B4, Residual_fit2_B4, 
...
```{r code=readLines("2d_lgcp_residuals_functions.R"), eval=FALSE, echo=TRUE}
# Do not change this code chunk
```
