...
flowchart TD
  ...
  res -->|"spatial<br/>covariate"| layer_eval
  eval_spatial --> input
...
classDiagram
    ...
    class cprod{
        ...
        cprod(...) with weight = weight.x * weight.y and sp::merge
        }
    class ips{
    ...
    class inla_mesh_1d{
        impute domain with inla.mesh.1d
...
Flow diagram for new integration scheme construction
...
samplers")]
domains --> calc_full_samplers["remove sampler
domains"] --> full_samplers[("full domain
samplers")]
samplers --> calc_full_samplers
domains --> multi_samplers
domains --> single_samplers
domains --> full_samplers

subgraph "multi sampler"
  ipoints_rowwise["compute ips
for each row"] -->
  ipoints_elementwise["compute ips
for each element"] -->
  ips_element[("ips list
for each row")] -->
...
multi_samplers -->|for each| ipoints_rowwise
