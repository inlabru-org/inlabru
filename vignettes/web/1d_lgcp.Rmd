...
Here we want to fit to the actual points, and the `inlabru` function that does this is called `lgcp` (for 'Log Gaussian Cox Process'). The `ips' and `domain` parameters are optional, one or the other or neither can be used. If neither are used, `lgcp` will infer the boundary of the `x` domain by using the upper and lower limits of the `x` column in `pts2`. Here are three ways of doing the same thing:
