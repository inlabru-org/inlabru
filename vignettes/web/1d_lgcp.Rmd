...
mdl <- x ~ spde1D(x, model = matern) + Intercept
```

Here we want to fit to the actual points, and the `inlabru` function that does this is called `lgcp` (for 'Log Gaussian Cox Process'). Either the `domain` or `ips' must be used. Here are twho ways of doing the same thing:
