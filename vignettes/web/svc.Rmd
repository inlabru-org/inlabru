...
```{r libs, warning=TRUE, message=TRUE, errors=TRUE}
# libraries
library(maps)
library(ggplot2)
library(sf)
...
count_dat <- download_dat %>%
  ...
  ungroup() %>%
  mutate(
    std_yr = year - max(year), obs = 1:nrow(.),
...
This qorks by an automatic creation of a `bru_mapper` object that knows how
...
`inla.mesh.2D()` and `inla.spde2.pcmatern()`, and the `inla.spde.make.index()`
and `inla.spde.make.A()` are only called internally by the `inlabru` code itself.
...
hull <- inla.nonconvex.hull(points = as.matrix(st_coordinates(count_dat)),
...
svc_components <- ~ -1 +
  ...
  alpha(st_coordinates, model = spde) +
  eps(st_coordinates, weights = log_hrs, model = spde) +
  tau(st_coordinates, weights = std_yr, model = spde)
# formula, with "." meaning "add all the model components":
svc_formula <- count ~ .
```
Specifying the function `st_coordinates` as input causes `inlabru` to extracts
the spatial coordinates of the observation locations from the internal `.data.`
object.  Alternatively, we can specify that explicitly as input, with
`st_coordinates(.data.)`, or with `cbind(easting, northing)`.
...
(`model="iid"`), globally zero-centered (sum to zero `constr=TRUE`), deviation 
...
the information necessary for posterior sampling (`config=TRUE`, but
this will otherwise be set by default by `bru()`, as `inlabru::predict()`
...
Note: some of the plots in this section can be simplified further by using
additional `inlabru` `ggplot` features.
...
epsi <- data.frame(
  median = exp(res$summary.random$eps$"0.5quant"),
...
# make a raster stack with the posterior median and range95
out_stk <- stack()
for (j in 1:3) {
  ...
  range95_j <- rasterFromXYZ(range95_j, crs = epsg6703km)
  out_j <- stack(mean_j, range95_j)
...
# alpha_s
alpha_spdf <- as(out_stk[["alpha_median"]], "SpatialPixelsDataFrame")
alpha_df <- as.data.frame(alpha_spdf)
colnames(alpha_df) <- c("value", "x", "y")
pa <- ggplot(states) +
  geom_sf(fill = NA) +
  coord_sf(datum = NA) +
  geom_tile(data = alpha_df, aes(x = x, y = y, fill = value)) +
  labs(x = "", y = "") +
  scale_fill_distiller("posterior\nmedian\nexp(alpha_s)", palette = "Spectral") +
  theme_bw() +
  geom_sf(fill = NA)

# epsilon_s
eps_spdf <- as(out_stk[["epsilon_median"]], "SpatialPixelsDataFrame")
eps_df <- as.data.frame(eps_spdf)
colnames(eps_df) <- c("value", "x", "y")
pe <- ggplot(states) +
  geom_sf(fill = NA) +
  coord_sf(datum = NA) +
  geom_tile(data = eps_df, aes(x = x, y = y, fill = value)) +
  labs(x = "", y = "") +
  scale_fill_distiller("posterior\nmedian\nepsilon_s", palette = "Spectral") +
  theme_bw() +
  geom_sf(fill = NA)

# tau_s
tau_spdf <- as(out_stk[["tau_median"]], "SpatialPixelsDataFrame")
...
pt <- ggplot(states) +
  ...
  labs(x = "", y = "") +
  scale_fill_distiller("posterior\nmedian\n100(exp(tau_s)-1)", palette = "Spectral") +
  theme_bw() +
  geom_sf(fill = NA)

# sites kappa_s median
ps <- ggplot(states) +
  geom_sf() +
  coord_sf(datum = NA) +
  geom_sf(data = site_map, size = 1, mapping = aes(colour = kappa$median)) +
  scale_colour_distiller("posterior\nmedian\nexp(kappa_s)", palette = "Spectral") +
  labs(x = "", y = "") +
  theme_bw() +
  geom_sf(fill = NA)

# alpha_s range95
alpha_spdf <- as(out_stk[["alpha_range95"]], "SpatialPixelsDataFrame")
alpha_df <- as.data.frame(alpha_spdf)
colnames(alpha_df) <- c("value", "x", "y")
pa_range95 <- ggplot(states) +
  geom_sf(fill = NA) +
  coord_sf(datum = NA) +
  geom_tile(data = alpha_df, aes(x = x, y = y, fill = value)) +
  labs(x = "", y = "") +
...
# epsilon_s range95
eps_spdf <- as(out_stk[["epsilon_range95"]], "SpatialPixelsDataFrame")
eps_df <- as.data.frame(eps_spdf)
colnames(eps_df) <- c("value", "x", "y")
pe_range95 <- ggplot(states) +
  geom_sf(fill = NA) +
  coord_sf(datum = NA) +
  geom_tile(data = eps_df, aes(x = x, y = y, fill = value)) +
  labs(x = "", y = "") +
...
# tau_s
tau_spdf <- as(out_stk[["tau_range95"]], "SpatialPixelsDataFrame")
tau_df <- as.data.frame(tau_spdf)
colnames(tau_df) <- c("value", "x", "y")
pt_range95 <- ggplot(states) +
  geom_sf(fill = NA) +
  coord_sf(datum = NA) +
  geom_tile(data = tau_df, aes(x = x, y = y, fill = value)) +
  labs(x = "", y = "") +
...
# sites kappa_s range95
ps_range95 <- ggplot(states) +
  geom_sf() +
  coord_sf(datum = NA) +
  geom_sf(data = site_map, size = 1, mapping = aes(colour = kappa$range95)) +
  scale_colour_distiller("posterior\nrange95\nexp(kappa_s)", palette = "Spectral") +
  labs(x = "", y = "") +
  theme_bw() +
  geom_sf(fill = NA)
