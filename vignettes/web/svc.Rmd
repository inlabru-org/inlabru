---
title: 'Building a Spatially Varying Coefficient Model in Continuous Space Using R-INLA'
...
with discreet-space analyses.

We build the model using a subset of the data described in Meehan et
al. (2019). Specifically, we use counts of American Robin 
(*Turdus migratorius*) from south central North America collected 
between 1987 and 2016 during the Audubon Christmas Bird Count (CBC). 
The overall goal of the analysis is to produce long-term relative 
abundance trends for robins that account for spatial and temporal 
variation in count effort.  
...
parameter. The expected count has a linear predictor:

$$log(\lambda_{st}) = \kappa_{s} + \alpha_s + \epsilon_{s}(log[Effort_{st}]) + \tau_{s}(Year_{st})$$
...
To get started with data analysis, we set up the environment, which
involves setting working directories, loading some packages, and
...
# libraries
library(maps); library(ggplot2); library(sf); library(raster)
library(tidyr); library(scales); library(inlabru); library(INLA)
library(dplyr)
...
# define a crs
epsg6703km <- paste("+proj=aea +lat_0=23 +lon_0=-96 +lat_1=29.5", 
                    "+lat_2=45.5 +x_0=0 +y_0=0 +datum=NAD83", 
                    "+units=km +no_defs")

# make a base map
states <- st_as_sf(maps::map("state", plot = FALSE, fill = TRUE)) %>% 
  filter(ID %in% c("texas", "oklahoma", "kansas", "missouri",
                   "arkansas", "louisiana")) %>% 
...
# get data
download_dat <- read.csv(paste0("https://raw.github.com/tmeeha/inlaSVCBC",
                  "/master/code/modeling_data.csv"))

# select subset
count_dat <- download_dat %>% 
  select(circle, bcr, state, year, std_yr, count, log_hrs, 
         lon, lat, obs) %>% 
  mutate(year=year+1899) %>% 
  filter(state %in% c("TEXAS", "OKLAHOMA", "KANSAS", "MISSOURI",
                      "ARKANSAS", "LOUISIANA"), 
         year>=1987) %>% 
  mutate(site_idx=as.numeric(factor(paste(circle, lon, lat)))) %>% 
  group_by(site_idx) %>% mutate(n_years=n()) %>% 
  filter(n_years>=20) %>% ungroup() %>% 
  mutate(std_yr=year-max(year), obs=1:nrow(.), 
...
A rough view of changes in robin relative abundance, whicht does not
...
# map it
ggplot() + 
  geom_sf(data=count_dat %>% filter(year_idx %in% seq(1, 30, 3)), 
          aes(col=log(count+1))) + 
  geom_sf(data=states, fill=NA) + coord_sf(datum = NA) +
...
later spatial modeling.

```{r spat_dat}
# make a set of distinct study sites for mapping
site_map <- count_dat %>% select(circle, lon, lat) %>%  
  distinct() %>% select(circle, lon, lat)

# save coordinates as matrices for later
unique_coords <- count_dat %>% select(circle, lon, lat) %>%  
  distinct() %>% select(lon, lat) %>%  st_drop_geometry %>% as.matrix()
...
we create a **modeling mesh**, which is considered a
representation of the continuous spatial surface based on
...
of the spatial terms in the model. Third, we create
...
the model. The four corresponding `R-INLA` functions for these steps are:
...
hull <- inla.nonconvex.hull(points=unique_coords, 200, 350)
mesh <- inla.mesh.2d(boundary=hull, max.edge=c(100, 600), # km inside and outside
                       cutoff=50, offset=c(100, 300)) # cutoff is min edge

# plot it
ggplot() + gg(data=mesh) +
  geom_sf(data=site_map, col="darkgreen", size=1) +
  geom_sf(data=states, fill=NA) + theme_bw() +
  labs(x="", y="")
```

## SPDE model object

Next we create an SPDE object to define the model smoothness, the 
...
al. 2018). If one wants to constrain this kind of spatial effect to sum 
to zero, `constr=T` should be added at this stage.

```{r spde}
# make spde
spde <- inla.spde2.pcmatern(mesh=mesh, prior.rang=c(500, 0.5), 
                            prior.sigma=c(1, 0.5)) 
```
...
A_eps <- inla.spde.make.A(mesh=mesh, loc=all_coords, 
                          weights=count_dat$log_hrs) # note weights argument
A_tau <- inla.spde.make.A(mesh=mesh, loc=all_coords, 
                          weights=count_dat$std_yr) # note weights argument
```
...
analysis using the `inla.stack()` function. This function is very
...
stack_fit <- inla.stack(
  ...
  effects = list(data.frame(intercept=1, 
                            kappa = count_dat$site_idx), # predictors, data frame 
                 alpha = alpha_idx,          # or index sets if spatial
                 eps = eps_idx,
                 tau = tau_idx),
  A = list(1,        # a value of 1 is given for non-spatial terms
           A_alpha,  
           A_eps, 
           A_tau)
)
...
It is possible to construct additional data stacks for creating 
predictions, and then stack the estimation and predictions stacks 
together for analysis. We do not do that in this example. Information 
on creating and adding prediction stacks in an SPDE context can be 
found in  Krainski et al. (2018, chapter 2) and Moraga (2019, chapter 
10).
...
# formula
svc_form <- count ~ -1 +
  f(kappa, model ="iid", constr = T, hyper = list(prec = pc_prec)) + 
  f(alpha, model = spde) + 
  f(eps, model = spde) + 
  f(tau, model = spde)
```
...
the site effect, as a normally distributed and exchangeable 
(`model="iid"`), zero-centered (sum to zero `constr=T`), deviation 
from $\alpha_s$. The name 'kappa' given in the `f()` argument is 
associated with the effect 'kappa' defined in the estimation stack. 
The second `f()` statement defines $\alpha_s$ as a spatially varying 
...
effect to sum to zero, `constr=T` should be added in the SPDE model 
definition rather than in the `f()` arguments. The third `f()` 
...
slope were specified during construction of the projector A matrix, 
`A_eps`. The fourth `f()` statement defines $\tau_s$ as an SVC for 
the year effect, with spatial structure described in the SPDE object. 
The weights for this spatially structured random slope were specified 
during construction of the projector A matrix, `A_tau`. Again, note 
that the current use of the term 'weights' is to define covariate 
values, as opposed to importance values.
...
and Empirical Bayes estimation. Another option is to use the Variational 
Bayes approximation as detailed in Van Niekerk and Rue (2021) and Van 
Niekerk et. al. (2022). The `inla()` run for this model takes about 1 
minute on a standard laptop computer. 

```{r run, eval=T, cache=TRUE}
res <- inla(svc_form,
            family="nbinomial",
            data=inla.stack.data(stack_fit), # stack the stack
            control.predictor=list(A=inla.stack.A(stack_fit), 
                                   compute=F), # must define A
            control.compute=list(waic=T, cpo=T, config=T),
...
starting with $\alpha_s$, which is effort-corrected relative abundance
at year = 0 (1987), given 1 hour of count effort (i.e., log[1]=0). 

```{r alpha_res}
summary(exp(res$summary.random$alp$mean)) # alpha
```
...
```{r tau_res}
summary((exp(res$summary.random$tau$mean)-1)*100) # tau
```
...
```{r map_grid}
# get lon and lat limits
xlim <- range(hull$loc[, 1])
ylim <- range(hull$loc[, 2])
grd_dims <- round(c(x=diff(range(xlim)), y=diff(range(ylim)))/25)

# make mesh projector that will get model summaries from the mesh to 
# the mapping grid
mesh_proj <- inla.mesh.projector(
    mesh, xlim=xlim, ylim=ylim, dims=grd_dims)
```

Then we populate the mapping grids with parameter estimates (posterior
mean and SD), turn them into a raster stack, and mask the raster stack
to the study area.

```{r map_vals}
# pull data
alph <- exp(res$summary.random$alp[, c(2:3)])
epsi <- res$summary.random$eps[, c(2:3)]
taus <- (exp(res$summary.random$tau[, c(2:3)]) - 1) * 100

# loop to get estimates on a mapping grid
pred_grids <- lapply(list(alpha=alph, epsilon=epsi, tau=taus),
                 function(x) as.matrix(inla.mesh.project(mesh_proj, x)))

# make a raster stack with the posterior mean and SD
out_stk <- stack()
for(j in 1:3){
  mean_j <- cbind(expand.grid(X=mesh_proj$x, Y=mesh_proj$y), 
            Z=c(matrix(pred_grids[[j]][,1], grd_dims[1])))
  mean_j <- rasterFromXYZ(mean_j, crs=epsg6703km)
  sd_j <- cbind(expand.grid(X=mesh_proj$x, Y=mesh_proj$y), 
            Z=c(matrix(pred_grids[[j]][,2], grd_dims[1])))
  sd_j <- rasterFromXYZ(sd_j, crs=epsg6703km)
  out_j <- stack(mean_j, sd_j)
  out_stk <- addLayer(out_stk, out_j)
}
names(out_stk) <- c("alpha_mean", "alpha_sd", "epsilon_mean", 
                    "epsilon_sd", "tau_mean", "tau_sd")
out_stk <- mask(out_stk, as(states, "Spatial"))
```

Finally, we plot the SVC for $\tau_s$ with the following code.

```{r svc_plots, warning=F, message=F, fig.width=8, fig.align='center'}
# tau_s
tau_spdf <- as(out_stk[[5]], "SpatialPixelsDataFrame")
tau_df <- as.data.frame(tau_spdf)
colnames(tau_df) <- c("value", "x", "y")
pt <- ggplot(states) + geom_sf(fill=NA) + coord_sf(datum = NA) +
  geom_tile(data=tau_df, aes(x=x, y=y, fill=value)) +
  labs(x="", y="") +
  scale_fill_distiller("posterior\nmean\ntau_s", palette="Spectral") +
  theme_bw() + geom_sf(fill=NA) 

# tau_s sd
tau_spdf <- as(out_stk[[6]], "SpatialPixelsDataFrame")
tau_df <- as.data.frame(tau_spdf)
colnames(tau_df) <- c("value", "x", "y")
psd <- ggplot(states) + geom_sf(fill=NA) + coord_sf(datum = NA) +
  geom_tile(data=tau_df, aes(x=x, y=y, fill=value)) +
  labs(x="", y="") +
  scale_fill_distiller("posterior\nSD\ntau_s", palette="Spectral") +
  theme_bw() + geom_sf(fill=NA) 
