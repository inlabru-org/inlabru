...
gcov <- terra::rast(system.file(gorillas_sf$gcov_file, package = "inlabru"))
```
...
ggplot() +
  gg(mesh) +
  geom_sf(data = boundary, alhpa = 0.1, fill = "blue") +
  geom_sf(data = nests) +
  ggtitle("Points")
```
...
pred$lambda <- terra::rast(
  as(sf::as_Spatial(pred$lambda), "SpatialPixelsDataFrame"),
)
pred$loglambda <- terra::rast(
  as(sf::as_Spatial(pred$loglambda), "SpatialPixelsDataFrame")
)

pl1 <- ggplot() +
  gg(pred$lambda, aes(fill = mean)) +
  geom_sf(data = boundary, alpha = 0.1) +
  ggtitle("LGCP fit to Points", subtitle = "(Response Scale)")

pl2 <- ggplot() +
  gg(pred$loglambda, aes(fill = mean)) +
  geom_sf(data = boundary, alpha = 0.1) +
...
ggplot() +
  gg(pred$lambda[[c("q0.025", "q0.5", "q0.975")]]) +
  facet_wrap(~lyr)
```
