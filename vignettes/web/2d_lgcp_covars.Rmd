...
mdl2 <- coordinates ~ vegetation(gcov$vegetation, model = "factor_full") - Intercept +
...
```{r warning=FALSE,message=FALSE,eval=TRUE}
lp2 <- predict(fit2, df, ~ mySmooth+vegetation)
lp2.spde <- predict(fit2, df, ~ mySmooth)
lp2.veg <- predict(fit2, df, ~ vegetation)
```
...
mdl3 <- coordinates ~ mySmooth(coordinates, model = pcmatern) + Intercept
fit3 <- lgcp(mdl3,
             data = nests,
             samplers = boundary,
...
```{r warning=FALSE,message=FALSE,eval=TRUE}
fn <- names(fit2$marginals.random$vegetation)
fn
...
```{r results="hide",warning=FALSE,message=FALSE,eval=TRUE}
e.int <- predict(efit, pixels(mesh), ~ exp(mySmooth + elev + Intercept))
...
e.lp <- predict(efit, pixels(mesh),  ~ mySmooth + elev) # SOLUTION
e.lp.elev <- predict(efit, pixels(mesh), ~ elev)
...
lprange <- range(e.lp$mean, e.lp.elev$mean, e.lp.spde$mean)
...
mdl <- x ~ beta_z(cov2_1D(x), model = "linear") +
  spde1D(x, model = inla.spde2.matern(mesh)) + Intercept
...
ggplot() +
  gg(prcov1D, color = "red") +
  geom_line(aes(x=prcov1Dspde$x,y=prcov1Dspde$median),col="blue",lwd=1.25) +
