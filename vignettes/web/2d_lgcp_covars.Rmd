...
function.  NOTE: There is a new method `eval_spatial()` that handles this
...
```{r results="hide",warning=FALSE,message=FALSE,eval=FALSE}
elev(f.elev(coordinates(.data.)), model = "linear")
```
...
e.int.log <- predict(efit, pixels(mesh, mask = boundary), ~ (mySmooth + elev + Intercept))
