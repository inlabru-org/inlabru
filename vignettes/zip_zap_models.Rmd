...
counts_rstr <- terra::rasterize(vect(nests), gcov, fun=sum, background=0) |> 
  terra::aggregate(fact=4, fun=sum) |> mask(vect(boundary))
plot(counts_rstr)
counts_rstr <- counts_rstr |> cellSize(unit="km") |> c(counts_rstr)
```
...
counts_df <- crds(counts_rstr, df=TRUE, na.rm=TRUE) |> 
  ...
  rename(count=sum) |> mutate(present=count>0) |> 
...
px_mesh <- fm_mesh_2d_inla(loc=st_as_sfc(counts_df),
                           boundary = boundary,
                           max.edge = c(0.5, 1), 
...
Now we can define the Poisson model which relates the species count per observation plot to the spatial covariates, such as the vegetation type and elevation. 

```{r fig-predict-poi, fig.width=7, fig.height=5, out.width="80%", fig.align='center'}
comps <-  ~ vegetation(gcov$vegetation, model="factor_contrast") + 
  elevation(eval_spatial(gcov$elevation, where=.data.), model="linear") +
  field(geometry, model=px_matern) + Intercept(1) 

fit_poi <- bru(comps,
              like(family = "poisson", data=counts_df,
                   formula = count~vegetation+elevation+field+Intercept,
                   E=area,
                   domain = list(geometry = px_mesh)))
summary(fit_poi)


lambda <- predict(fit_poi, fm_pixels(px_mesh, mask=boundary),
                  ~ exp(vegetation+elevation+field + Intercept))

ggplot() +
  geom_fm(data = px_mesh) +
  gg(lambda, geom = "tile") +
  #geom_sf(data=nests, size=1, color="firebrick", pch=4) +
...
fit_zip <- bru(comps,
                like(family = "zeroinflatedpoisson1", data=counts_df,
                     formula = count~vegetation+elevation+field+Intercept,
                     domain = list(geometry = px_mesh)))
...
lambda <- predict(fit_zip, fm_pixels(px_mesh, mask=boundary),
                  ~ exp(vegetation+elevation+field + Intercept))
...
This model is much better at differentiating between the sites and produces sensible predictions.

## ZAP model

Judging by the distribution of nests in relation to the [spatial covariates](articles/2d_lgcp_covars.Rmd), gorillas do not like to set their nests in certain vegetation. The type of vegetation may not influence the density of the nests, but it definitely drives the presence/absence. In this case the `vegetation` covariate should be included in the binomial part of the model, but not the Poisson part. 
...
Here ZIP distribution only governs the positive counts and the absences will be handled by a separate binomial model which can have its own covariates. Note, that we exclude the observations with absent nests from the ZIP model by subsetting the data to include only observations where `present` is `TRUE`.

```{r fig-pred-zap, fig.cap="Predictions from zero-adjusted model",fig.width=7, fig.height=5, out.width="80%", fig.align='center'}
fit_zap <- bru(comps,
               like(family = "binomial", data=counts_df,
                    formula = present~vegetation+field+Intercept,
                   domain = list(geometry = px_mesh)),
               like(family ="zeroinflatedpoisson0",
                    data=counts_df[counts_df$present,],
                    formula = count~elevation+field+Intercept,
                    domain = list(geometry = px_mesh),
                    control.family = list(hyper=list(theta=list(initial=-20, fixed=TRUE)))))
summary(fit_zap)

lambda <- predict(fit_zap, fm_pixels(px_mesh, mask=boundary),
                  ~ exp(vegetation+elevation+field + Intercept))

ggplot() +
  geom_fm(data = px_mesh) +
  gg(lambda, geom = "tile") +
  geom_sf(data=nests, color="firebrick", size=1, pch=4, alpha=0.1)+
  ggtitle("Nest intensity per km squared")

```

In order to make sure that Poisson distribution will be properly truncated, we include the `control.family` argument with the parameter `theta` (which gets translated into the probability $p$) is fixed to a large negative value, such that when transformed by $\ln(\theta)=p$ it would end up close to zero.
